...
\item[names()] gives names for the elements,
...
the document (e.g., a file path or a \acronym{URL}; \code{NA} if not applicable or
unavailable).
...
VecSource <- function(x)
    SimpleSource(length = length(x), names = names(x), content = as.character(x),
                 class = "VecSource")
@
...
generator. A function generator is indicated by inherting from class
