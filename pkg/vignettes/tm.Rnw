...
relevant terms in text mining. Character vectors may be passed over to the
\code{DocumentTermMatrix()} constructor. Then the created matrix is tabulated
against the dictionary, i.e., only terms from the dictionary appear in the
matrix. This allows to restrict the dimension of the matrix a priori and to
focus on specific terms for distinct text mining contexts, e.g.,
<<>>=
