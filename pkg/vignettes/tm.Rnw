...
like a database). \code{print()} gives a concise overview whereas the full
content of text documents is displayed with \code{inspect()}.
<<>>=
inspect(ovid[1:2])
@
...
<<>>=
reuters <- tm_map(reuters, genMap(tolower))
@
As you see you can use arbitrary character processing functions as
transformations as long as the function returns a text document. In this case we
use \code{genMap()} which provides a convenience wrapper to access and set the
content of a document. Consequently most text manipulation functions from base
\proglang{R} can directly be used with this wrapper. This is true for
